...
use crate::input::{CreateOrderInput, DeleteOrder, Side};

#[derive(Serialize, Deserialize, Debug,)]
pub struct Orderbook {
    pub bids: HashMap<String, Vec<UserOrder>>,
    pub asks: HashMap<String, Vec<UserOrder>>,
...
impl Orderbook {
    pub fn new() -> Self{
        Self { bids: HashMap::new(), asks: HashMap::new(), order_id_index:0 }
    }
}
...
impl Orderbook {
    ...
    pub fn get_depth(&self) -> DepthResponse {
        ...
        for (price, orders) in self.bids.iter() {
            bids.push(Depth { price: price.parse().unwrap(), quantity: orders.iter().map(|o| o.qty).sum() });
        }
        for (price, orders) in self.asks.iter() {
            asks.push(Depth { price: price.parse().unwrap(), quantity: orders.iter().map(|o| o.qty).sum() });  
        }
